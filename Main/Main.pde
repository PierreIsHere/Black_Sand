...
void setup() {
  size(1600, 900, P2D);
  //fullScreen(P2D);
  c = new Client(this, "192.168.0.12", 12345); 
  t = new Tank(width+100, height-100, 75, "tanks/bTank1.png");
  t2 = new Tank(100, height-100, 75, "tanks/rTank1.png");
...
void sendKeyStroke() {
  if (keyPressed) {
    if (key == CODED) {
      if (keyCode == UP) c.write(c.ip()+"w");
      if (keyCode == DOWN) c.write(c.ip()+"s");
      if (keyCode == LEFT) c.write(c.ip()+"a");
      if (keyCode == RIGHT) c.write(c.ip()+"d");
    }
    if (key == ' ') c.write(c.ip()+"x");
  }
}
...
void controlP2() {
  if (c.available()>0) {
    ...
    input = c.readString(); 
    //if (!input.startsWith(c.ip())) {
      t2.webKey(input.charAt(input.trim().length()-1));
      println(input.charAt(input.trim().length()-1));
    //}
  }
}
